...
Please select the option that aligns with your daily activity</p>
    ...
    <div className="flex justify-around mt-5 ml-5 mr-5">
          <Link className="bg-green-400  text-white text-xl pl-7 pr-7 pt-1 pb-1 rounded-sm" to={"/DailyMeal_yes"}>Back</Link>
          {isFieldSelected ? (
            <Link className="bg-green-400  text-white text-xl pl-7 pr-7 pt-1 pb-1 rounded-sm" to={"/sqs_yes"} onClick={handleNext}>
              Next
            </Link>
          ) : (
...
export default ActivityLevel_yes;
